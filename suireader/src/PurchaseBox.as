package
{
    ...
    /**
     * Purchase virtual goods box
	 * @author Ted
     */
    public class PurchaseBox extends PetiPanel
    {
		private var _vgs:Array;
		private var _rowheight:int = 60;
		private var _topmargin:int = 8;
		private var _itemchecks:Array;
		private var _buyone:Boolean;
		
        public function PurchaseBox(vgs:Array,buyone:Boolean=true,w:int=400,h:int=380)
        {
            super(w,h,false,true);
            
            addText('Required Items', { 'size':20, 'color':0xffff00, 'bold':true } );
			addText('Must buy at least one to read on');
            addSeparator();
        
			_vgs = vgs;
			_itemchecks = new Array();
			_buyone = buyone;
			
            for (var i:int=0,vg:Object; vg=vgs[i]; i++){
                addrow(i, vg.id, vg.name, vg.price, vg.note);
            }
        ...
        private function addrow(idx:int, id:int, vgname:String, vgprice:Number, vgnote:String):void
        {
			...
			ico.name = idx.toString();
            left.addControl(ico);
			ico.addEventListener(MouseEvent.CLICK, onSelectItem);
            var dm:DataManager = DataManager.instance;
            dm.loadImage('/mm/vg_'+id, function(img:Loader):void{
                ico.addChild(img);
			...
			if (_buyone)
				chbx.addEventListener(MouseEvent.CLICK, onCheckBoxClick);
		...
		{
			for (var i:int = 0, c:PetiCheckbox; c = _itemchecks[i] as PetiCheckbox; i++) {
		...
		{
			var x:int = parseInt(e.currentTarget.name);
			if (_buyone) {
				for (var i:int = 0, c:PetiCheckbox; c = _itemchecks[i] as PetiCheckbox; i++) {
					c.checked = (i == x);
				}
			} else {
				_itemchecks[x].checked = !_itemchecks[x].checked;
			}
		}
        ...
        {
            var itms:Array = [];
            for (var i:int=0,bb:PetiCheckbox; bb=_itemchecks[i]; i++){
                if(bb.checked)
                    itms.push(_vgs[i].id);
            }
            trace('purchase ', itms);
