package com.suinova.sgapi 
{
	...
	public class Environment
	{
         ...
         * @param   appid : suireader,suigallery,suiquest,etc
		 * @param	callback
		 */
		public function loadEnv(appid:String,callback:Function):void
		{
			if (_env == null) {
				if (ExternalInterface.available) {
					try {
						ExternalInterface.addCallback('jsCallFlash', onJsCallFlash);
						calljs('getenv', {app:appid}, callback);
					} catch (err: Error) {
						trace(err.message);
						...
						_env = { book:'291',page:0,host:'http://localhost:8084',cookie:'' };
						callback(env);
					}
				} else {
		...
		/**
		 * Donate some points to a page author.
		 * @param	params - {pid:page_id, pts:points_to_donate}
		 * @param	callback
		 */
		public function donate(params:Object, callback:Function):void
		{
			if (ExternalInterface.available) {
				try {
					calljs('donate', params, callback);
				} catch (err: Error) {
					trace(err);
					callback( { 'error':err.message } );
				}
		...
		public function loadImage(url:String, callback:Function):void
		{
			...
			loader.contentLoaderInfo.addEventListener(Event.COMPLETE, function(e:Event):void {
					trace('Environment.loadImage('+url+') Image loaded');
					callback(loader);
				});
			loader.contentLoaderInfo.addEventListener(IOErrorEvent.IO_ERROR, function(e:IOErrorEvent):void {
					trace("Environment.loadImage('+url+')  IO_Error: ", e);
					//throw e;
					callback(null);
				});
		}
